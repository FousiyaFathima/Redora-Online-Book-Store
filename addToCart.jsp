<%-- 
    Document   : addToCart
    Created on : 28 Mar, 2026, 10:56:07 AM
    Author     : acer
--%>
...
// Get parameters from category page
...
try{
    ...
    if(rs.next()){
        // If exists, **add the new quantity to existing quantity**
        int existingQty = rs.getInt("qty");
        ps.close();
        String updateSql = "UPDATE cart SET qty=? WHERE email=? AND book=?";
        ps = con.prepareStatement(updateSql);
        ps.setInt(1, existingQty + qty);  // sum of old + new
        ps.setString(2,email);
        ps.setString(3,book);
        ps.executeUpdate();
        ps.close();
    } else {
        // If not exists, insert new record with exact quantity
        ps.close();
...
// Redirect directly to cart page to see updated quantity
